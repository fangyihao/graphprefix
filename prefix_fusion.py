...
def construct_model(args, kg):
    ...
    model = get_model(args, TaskType.MULTIPLE_CHOICE, config)
    '''
...
def calc_eval_accuracy(args, eval_set, model, loss_type, loss_func, debug, save_test_preds, preds_path):
    ...
    with torch.no_grad():
        for qids, labels, *input_data in tqdm(eval_set, desc="Dev/Test batch"):
            ...
            outputs = model(*get_model_inputs(args, input_data))
            logits = outputs.logits
            
            # TODO
            labels = labels.cuda(0)
            logits = logits.cuda(0)
...
def get_model_inputs(args, inputs):
    input_ids, attention_mask, token_type_ids, output_mask, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask, edge_index, edge_type = inputs
    if args.prefix_cross_attention:
        return input_ids, token_type_ids, attention_mask, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask, edge_index, edge_type
    else:
...
def train(args, resume, has_test_split, devices, kg):
    ...
    print("len(dataset.tokenizer):", len(dataset.tokenizer))
    ...
    '''
    loaded_roberta_keys = model.parameters()
    ...
    model.to(devices[0])
    try:
        model.concept_emb.to(devices[1])
        model.edge_encoder.to(devices[1])
        model.emb_node_type.to(devices[1])
        model.emb_score.to(devices[1])
        for gnn in model.gnn_layers:
            gnn.to(devices[1])
    except:
        print("No graph layers")
    ...
    for epoch_id in trange(last_epoch + 1, args.n_epochs, desc="Epoch"):
        ...
        for qids, labels, *input_data in tqdm(train_dataloader, desc="Batch"):
            ...
            for a in range(0, bs, args.mini_batch_size):
                ...
                # TODO
                outputs = model(*get_model_inputs(args, [x[a:b] for x in input_data]))
                logits = outputs.logits
                # logits: [bs, nc]
        ...
        dev_total_loss, dev_acc = calc_eval_accuracy(args, dev_dataloader, model, args.loss, loss_func, args.debug, not args.debug, preds_path)
        if has_test_split:
            preds_path = os.path.join(args.save_dir, 'test_e{}_preds.csv'.format(epoch_id))
            test_total_loss, test_acc = calc_eval_accuracy(args, test_dataloader, model, args.loss, loss_func, args.debug, not args.debug, preds_path)
        else:
            test_acc = 0
...
def evaluate(args, has_test_split, devices, kg):
    ...
    preds_path = os.path.join(args.save_dir, 'dev_e{}_preds.csv'.format(epoch_id))
    dev_total_loss, dev_acc = calc_eval_accuracy(args, dev_dataloader, model, args.loss, loss_func, debug, not debug, preds_path)
    if has_test_split:
        # Evaluation on the test set
        preds_path = os.path.join(args.save_dir, 'test_e{}_preds.csv'.format(epoch_id))
        test_total_loss, test_acc = calc_eval_accuracy(args, test_dataloader, model, args.loss, loss_func, debug, not debug, preds_path)
    else:
        test_acc = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument("--prefix_cross_attention", default=True, type=bool, help="Will use prefix cross attention during training")
