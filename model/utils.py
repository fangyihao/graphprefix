...
from model.multiple_choice import (
    ...
    RobertaPrefixCrossAttentionForMultipleChoice
)
...
PREFIX_CROSS_ATTENTION_MODELS = {
    "roberta": {
        TaskType.MULTIPLE_CHOICE: RobertaPrefixCrossAttentionForMultipleChoice,
    }
}
...
def get_model(model_args, task_type: TaskType, config: AutoConfig, fix_bert: bool = False):
    if model_args.prefix_cross_attention:
        config.hidden_dropout_prob = model_args.hidden_dropout_prob
        config.pre_seq_len = model_args.pre_seq_len
        config.prefix_projection = model_args.prefix_projection
        config.prefix_hidden_size = model_args.prefix_hidden_size
        config.n_ntype = model_args.n_ntype
        config.n_etype = model_args.n_etype
        config.gnn_dim = model_args.gnn_dim
        config.k = model_args.k
        
        model_class = PREFIX_CROSS_ATTENTION_MODELS[config.model_type][task_type]
